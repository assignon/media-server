...
if($chek_name->rowCount() == 0)
{
    ...
    if(!empty($allDayVal) AND !empty($invites))
    {
      ...
      $insertEvt->execute(array($eventName, $eventLocation, $eventStart, $eventTimeStart, $eventEnd, $eventTimeEnd, $allDayVal));
      ...
      $insertEvt = $pdo->prepare('INSERT INTO events(name, location, start_date, start_time, end_date, end_time, all_day) VALUES(?,?,?,?,?,?,?)');
      $insertEvt->execute(array($eventName, $eventLocation, $eventStart, $eventTimeStart, $eventEnd, $eventTimeEnd, "Not allday"));
   ...
   echo 'This event name already exist...';
